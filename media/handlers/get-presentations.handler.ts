...
export class GetPResentationsHandler implements CommunicationHandler {
    ...
    public async processCommunication(communication: ICommunication<any>): Promise<ICommunication<any>> {
        console.log("get-presentations.handler.ts ProcessCommunication()");
        let response: any;
        ...
        try {
            switch (communication.head.verb) {
                case CommandVerbs.get:
                    // response = await this.documentService.getPresentations(communication.body);
                    break;
                default:
                    break;
            }
